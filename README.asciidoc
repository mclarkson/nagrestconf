...
Point the worker at the master nagios.

----
MASTER_IP_ADDR=10.1.1.45
...
If the Master nagios is *running in a chroot* allow svn and ssh to use the chroot port.
...
----
ssh -p 2222 svnsync@10.1.1.45 ls /
----

A root directory listing MUST succeed before running slc_configure below.

*Log into 10.1.1.45* normally as root now and run the following code,
