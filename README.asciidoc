...
06 Jan 2013 New version 1.164 (redhat 5/6 and debian packages released to Sourceforge)
...
*UPGRADING TO VERSION 1.164*

Encoding for the commands table has changed. After the upgrade change all
commands that have four backslashes, '\\\\', to two backslashes, '\\'.

If using vim, do:

 cd /etc/<nagios_dir>/objects/<FOLDER>/setup
 cp <FOLDER>_commands.setup ~
 vim <FOLDER>_commands.setup

Replacing <FOLDER> and <nagios_dir> with the correct path names, then, in vim:

 :%s/\\\\/%5C%5C/g
 :%s/%5C%5C%5C%5C/%5C%5C/g
 :wq
