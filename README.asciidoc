...
* Jquery and theme update. Adding tabs to edit dialogs to support extra options.
...
----
# Define this function (copy and paste everything)
...
#
# Only do the next lines once (and once only!)
#

FOLDER="local"; FILE="servicesets";
...
#
# Sanity check then move:
mv /tmp/${FOLDER}_${FILE}.setup /etc/nagios/objects/${FOLDER}/setup/${FOLDER}_${FILE}.setup

FOLDER="local"; FILE="services";
while IFS=, read c1 c2 c3 c4 c5; do echo -n "$c1,$c2,"; urlencode "$c3"; echo -n ","; urlencode "$c4"; echo ",$c5"; done </etc/nagios/objects/${FOLDER}/setup/${FOLDER}_${FILE}.setup >/tmp/${FOLDER}_${FILE}.setup
sed -i "s/%60/%2C/g" /tmp/${FOLDER}_${FILE}.setup
#
# Sanity check then move:
...
Run the following command on the nagios worker that needs to be exported from,
changing the FOLDER variable only:
...
Copy ~/script_vars to a linux box with curl on it.

Create a configuration file. TODO
...
First, install the operating system. The following instructions were tested on
RedHat 5.8.

If this is a partitioned system then run the following:
...
----
yum install nagrestconf nagios-www smorg-nagios-plugins \
            smorg-nagios-plugins-extra smorg-nrpe smorg-nrpe-plugin
----

Create the base nagios configuration:
