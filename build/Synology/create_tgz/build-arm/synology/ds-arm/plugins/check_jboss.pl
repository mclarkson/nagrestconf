...
my $twiddle = "/usr/local/jboss/bin/twiddle.sh";
my $tempdir = "/tmp";
...
### IE amended 30/09/2010 - added check so if 'name' is specified in mbean then syntax is corrected... 
if (($o_jmxmbean =~ /name=/)) {$twcall .= ",type=" . $o_datatype if $o_datatype;} else {$twcall .= ":type=" . $o_datatype if $o_datatype;}
### $twcall .= ":type=" . $o_datatype if $o_datatype;
### IE amended 30/09/2010 - added check so if 'service' is specified in mbean then syntax is corrected...
if (($o_jmxmbean =~ /name=/)) {$twcall .= ",service=" . $o_servicetype if $o_servicetype;} else {$twcall .= ":service=" . $o_servicetype if $o_servicetype;}
